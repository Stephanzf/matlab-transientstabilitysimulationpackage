 ...
 * $Revision: 1.8 $  $Date: 1995/02/17 13:11:13 $
 */
...
#define mxCreateString(str)  (Matrix *)(mxSetPr(mxCreateFull(1,strlen(str),0),str)-2)
/* Actually the following won't be called since it's been guarded by
   MATLAB_MEX_FILE. But it needs to be redefined for compiling. */
#define mexCallMATLAB(a,b,c,d,e) fisError("User-defined functions for FLT are not allowed in RTW!\n");
#endif
...
{
	int i;
	Matrix *PARA = mxCreateFull(MF_PARA_N, 1, 0);
	Matrix *X = mxCreateFull(1, 1, 0);
	Matrix *OUT;
	double out;
	Matrix *prhs[2];
	...
	mexCallMATLAB(1, &OUT, 2, prhs, mf_type);
	out = mxGetScalar(OUT);
...
#endif
{
	int i;
	Matrix *PARA = mxCreateFull(MF_PARA_N, 1, 0);
	Matrix *X = mxCreateFull(leng, 1, 0);
	Matrix *OUT;
	Matrix *prhs[2];
...
{
	double out;
	Matrix *VARIABLE = mxCreateString(variable);
	Matrix *OUT;
...
#endif
{
	Matrix *X, *Y, *OUT, *prhs[2];
	int i;

	/* allocate memory */
	X = mxCreateFull(m, n, 0);
	Y = mxCreateFull(m, n, 0);
	prhs[0] = X;
	prhs[1] = Y;
	...
	/* call matlab function */
	mexCallMATLAB(1, &OUT, 2, prhs, func);
...
#endif
{
	...
	Matrix *MF_ = mxCreateFull(leng, 1, 0);
	Matrix *OUT;
	Matrix *prhs[2];
	int i;
	...
	prhs[0] = X;
	prhs[1] = MF_;
	mexCallMATLAB(1, &OUT, 2, prhs, defuzz_fcn);
	out = mxGetScalar(OUT);
